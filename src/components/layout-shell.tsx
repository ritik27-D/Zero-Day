...
interface LayoutShellProps {
  ...
  activeNav?: string;
  adminNavContent?: React.ReactNode;
}

export default function LayoutShell({ children, activeNav, adminNavContent }: LayoutShellProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col antialiased text-slate-900 overflow-x-hidden w-full">
      ...
      <header className="site-header sticky top-0 z-40 border-b backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between min-h-[66px] sm:min-h-[70px] py-1.5 gap-2 lg:gap-3 xl:gap-4">
            ...
            <nav className={isAdminRoute ? "hidden" : "hidden lg:flex items-center gap-0.5 xl:gap-1"}>
              {navLinks.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={`site-nav-link px-3 py-2 rounded-md text-sm font-semibold tracking-tight ${
                      isActive
                        ? "text-white"
                        : ""
                    }`}
            ...
            {/* Right: Researcher sign-in or admin indicator */}
            <div className="flex items-center gap-2 sm:gap-3 shrink-0">
              {isAdminRoute ? (
                ...
                <Link
                  href="/login"
                  className="inline-flex items-center justify-center px-3 py-2 sm:px-3.5 rounded-md text-sm font-bold text-white bg-slate-900 hover:bg-slate-800 transition shadow-xs cursor-pointer whitespace-nowrap"
                >
                  Researcher Sign In
                </Link>
              )}
              ...
              <button
                ...
                className={`${isAdminRoute ? "hidden" : "lg:hidden"} p-1.5 sm:p-2 rounded-lg text-slate-600 hover:bg-slate-100 hover:text-slate-900`}
        ...
        {mobileMenuOpen && !isAdminRoute && (
          <div className="lg:hidden border-t border-slate-200 bg-white px-4 pt-3 pb-6 space-y-4 shadow-lg max-h-[85vh] overflow-y-auto animate-in slide-in-from-top-2 duration-150">
                ...
                return (
                  <div key={item.id} className="pt-1.5 first:pt-0">
                    <Link
                      ...
                      data-active={isActive}
                      className={`site-nav-link block py-2.5 text-sm font-bold transition ${
                        isActive ? "text-[#0f766e]" : "text-[#274c5e]"
                      }`}
                    >
                      {item.label}
                    </Link>
            ...
            <div className="pt-2 border-t border-slate-100 grid grid-cols-2 gap-2 text-sm text-slate-600">
            ...
            <div className="pt-3 border-t border-slate-200">
              {isAdminRoute ? (
                <Link
                  href="/admin"
                  onClick={() => setMobileMenuOpen(false)}
                  className="block w-full text-center px-4 py-2.5 rounded-md text-sm font-bold text-white bg-slate-900"
                >
                  Admin Dashboard
                </Link>
              ) : (
                <Link
                  href="/login"
                  onClick={() => setMobileMenuOpen(false)}
                  className="block w-full text-center px-4 py-2.5 rounded-md text-sm font-bold text-white bg-slate-900 hover:bg-slate-800 transition"
                >
                  Researcher Sign In
                </Link>
              )}
      ...
      {/* ========================================================================= */}
      <footer className={isAdminRoute ? "hidden" : "site-footer mt-auto"}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 lg:gap-10">
            ...
            {/* Col 2: Quick Links */}
            <div>
              <h3 className="text-sm font-bold uppercase tracking-wider text-slate-900 mb-3.5">
                Quick Links
              </h3>
